...
class PCT_Reports(tk.Frame):
    ...
    def display_report(self,weekdates,wtitle):
        rptdata = PCT_TimeDB.PCT_TimeDB.generate_report(self.DB_Handle,weekdates)
        c0 = "#DDD5D5"
        c1 = "#C9C2C2"
        colr = True
        rptdict = {}
        for onerow in rptdata:
            ...
            if nm in rptdict:
                num = rptdict[nm]
                num[ix] = timn
                num[8] = num[1]+num[2]+num[3]+num[4]+num[5]+num[6]+num[7]
                rptdict[nm] = num
            else:
        ...
        for dt in weekdates:
            c += 1
            daywk = dt + '\n' + self.wkday[c]
            tk.Label( self.holdrtl, text=daywk, width=10, anchor='w').grid(row=r,column=c)
        c += 1 
        tk.Label( self.holdrtl, text='Lawyer Total\nReal Total', width=10, anchor='w').grid(row=r,column=c)
        ...
        for drk,drv in sorted(rptdict.items(), key=lambda i: i[1][0]):
            r += 1
            c = 0
            tk.Label( self.holdrtl, text=drk, width=lablen, anchor='w').grid(row=r,column=c)
            for tim in drv[1:]:
                daytotix = c
                c += 1
                tenthtime = round(math.ceil(tim / (360))/10,1)
                if c == len(dayltot):
                    dtim = str(tenthtime) + '/' + "{:03.2f}".format(tim/3600.0)
                    tk.Label( self.holdrtl, text=dtim, width=10, anchor='w').grid(row=r,column=c)
                else:
                    colr = False if colr else True
                    tk.Label( self.holdrtl, text=str(tenthtime), width=10, anchor='w',bg=c0 if colr else c1).grid(row=r,column=c)
                dayrtot[daytotix] = dayrtot[daytotix] + tim
                dayltot[daytotix] = dayltot[daytotix] + tim
        r += 1
        c = 0
        tk.Label( self.holdrtl, text='Lawyer Total\nReal Total', width=lablen, anchor='w').grid(row=r,column=c)
        for ttim in dayltot:
            c += 1
            rix = c - 1
    ...
    def notes_setup(self,pct,dbhn,dbht,rpt):
        ...
        self.limitdt = str((date.today() + timedelta(days=-(self.rptwks * 7))).isoformat())
