...
class PCT_TimeDB:
    ...
    def get_config(self,which=None):
        if which == None:
            self.cur.execute("select config_nm, config_val_cd, config_nm_desc_tx from t_task_config order by config_nm")
        else:
    ...
    def validate_create_config_paths(self):
        ...
        self.cleandict['.log'] = '.' # logging starts before database opens, so in program directory
    ...
    def update_task_sort(self,opt,tid,srtno):
        if opt == 'RPT':
            ...
            self.cur.execute("update t_tasks set task_sort_gui_no = ? where task_id = ?", (srtno,tid))
    ...
    def get_todaydt_time(self):
        self.cur.execute("select coalesce(sum(task_time_no),0) from t_task_time where task_time_dt = ?",[self.todaydt])
        todaydt_sec = self.cur.fetchone()[0]
        return todaydt_sec
    def get_next_timelog_key(self):
        self.cur.execute("select coalesce(max(timelog_id) + 1,0) from t_task_timelog")
    ...
    def adjust_aftertime_value(self):
        ...
        for walltime in times:
            ...
            wallsecdiff = int((datetime.strptime(walltime[3], FMT) - datetime.strptime(walltime[2], FMT)).total_seconds())
            ...
            if wallsecdiff > after_use:
                ...
                logging.info("AFTERTIME skipping current difference time is less than 3600 seconds " + str(wallsecdiff))
        try:
            ...
            logging.info("CLOCKSLOW clockslow ratio app/wall " + str(clockslow))
        except ZeroDivisionError:
            clockslow = 1
            logging.info("AFTERTIME not adjusted no wall time captured")
        new_aftertime = int(float(current_aftertime) * clockslow)
        logging.info("AFTERTIME calculated clock difference " + str(total_wall_tm - total_appl_tm))
    ...
    def run_migrations(self,ver):
        ...
        if pre7[0] < 3:
            ...
            lcur.executescript("""
                ...
                Commit Transaction;            
            """)
        lcur.execute("SELECT CONFIG_NM, CONFIG_VAL_CD from T_TASK_CONFIG WHERE CONFIG_ID = 3")
        (dbvernm,dbvercd) = lcur.fetchone()
        logging.info('Database version in is ' + dbvercd)
        if dbvernm == 'DBVERSION': # subsequent migration runs, need to check ver to dbvercd
...
class PCT_NotesDB:
    ...
    def getNotesby(self,start_dt,end_dt,taskdbhandle,rpttype):
        ...
        for task in tasks:
            self.cur.execute("INSERT INTO TEMP.G_TASKS (TASK_SORT_RPT_NO ,TASK_PROJ_NM ,TASK_NM ,TASK_ID) VALUES (?,?,?,?)",task)
        if rpttype == 'D':
