...
class PCT_Notes(tk.Frame):
    ...
    def editnote(self, pct, dbnhandle, dbthandle, keys):
        ...
        self.tid = keys[0]
        prevnotesshow = int(
            self.TasksDB_Handle.get_config_item('PREVNOTESSHOW'))
        ...
        self.but.append(
            tk.Button(self.holdnotetl, image=self.butphoto[-1], command=self.process_note))
        self.but[-1].grid(row=r, column=c)
        ...
        if len(notev) > 0:
            for nr in notev:
                textv.append(nr)
        self.textPad = tkst.ScrolledText(
            master=self.holdnotetl, width=w, height=he, wrap=tk.WORD)
        r += 1
        ...
        for ix, txt in enumerate(textv):
            ...
            viewPadL.append(
                tkst.ScrolledText(self.holdnotetl, width=w, height=hv, wrap=tk.WORD))
            r += 1
            viewPadL[-1].grid(row=r + ix, column=c)
