...
class PCT_PunchClock(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        totaltimemsg = 'Starting Total time is ' + \
    ...
    def about(self):
        ...
        l2 = tk.Label(t, anchor='w', text="by Ed Lipson (edlipsongm@gmail.com)").grid(
            row=1, column=0)
        l3 = tk.Label(t, anchor='w', text="Concept from Project Clock").grid(
            row=2, column=0)
        l4 = tk.Label(t, anchor='w', text="  by David Keeffe @2000").grid(
            row=3, column=0)
        l5 = tk.Label(t, anchor='w', text="Licensed under Apache License 2.0").grid(
            row=4, column=0)
    ...
    def adjust_geo(self, rows):
        ...
        logging.info("GEO old " + oldgeo + " new " + newgeo +
                     " max but len " + str(self.max_but_len))
    ...
    def addbuttons(self):
        ...
        b = tk.Button(
            self.f, image=self.butphotos[-1], anchor='w', command=self.quit)
        self.buttons.append(b)
        ToolTip.ToolTip(
            self.buttons[-1], anchor='e', text="Exit the application")
        self.butphotos.append(tk.PhotoImage(file='stop.gif'))
        b = tk.Button(
            self.f, image=self.butphotos[-1], anchor='e', command=self.stop)
        self.buttons.append(b)
        ToolTip.ToolTip(
            self.buttons[-1], anchor='e', text="Stop all of the timers")
        self.butphotos.append(tk.PhotoImage(file='silent.gif'))
        self.buttons.append(tk.Button(
            self.f, image=self.butphotos[-1], anchor='e', command=self.togglealarm))
        ToolTip.ToolTip(
            self.buttons[-1], anchor='e', text="Quiet Toggle of the Task alarm if one exists. Blue on, Yellow off, white none")
        c = -1
    ...
    def quit(self):
        ...
        self.master.destroy()
        totaltimemsg = 'Ending Total time is ' + \
            str(timedelta(seconds=self.totaltime)) + \
            ' or ' + str(self.totaltime)
        logging.info(totaltimemsg)
        ...
        if self.alarmcycletime != None:
            ...
            if self.alarmcountdown < 1:
                ...
                if self.popuppopped == None:
                    ...
                    t.iconbitmap('digitalclock2.ico')
                    l1 = tk.Label(t, anchor='w', text="PCT Puch CLock Timer").grid(
                        row=0, column=0)
                    l2 = tk.Label(t, anchor='w', text="Verify Active Task is Proper").grid(
                        row=1, column=0)
                    self.popuppopped = tk.Button(
                        t, text='Dismiss', command=lambda lt=t: self.alarm_clear(lt))
    ...
    def adjust_task_time(self, ix, atim):
        if ix == self.runningix:
            ...
            if self.timlist[ix] == 0:
                ...
                self.tasklist[ix] = (
                    tid, new_ttid, pnm, tnm, tat, tash, tast, tsrt, atim if atim > 0 else 0, tdt)
    ...
    def display_tasks(self, gridrow, list):
        for t in list:
            ...
            tl = tk.Label(
                self.f, text=self.dtimlist[-1].get(), width=9, anchor='e')
