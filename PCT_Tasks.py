...
class PCT_Tasks(tk.Frame):
    ...
    def display_hidden(self,pct):
        ...
        if len(self.h_list) > 0:
            ...
            self.cbpb = tk.Button(self.holdctl, image=self.processphoto, command=self.process_display_hidden )
    ...
    def process_display_hidden(self):
        ...
        if change > 0:
            for pop in reversed(poplist):
                self.h_list.pop(pop)
            punchclock.PCT_PunchClock.set_tasklists(self.holdpct,[self.d_list,self.h_list,[change]])
    ...
    def add_new_task(self,pct,dbhandle):
        self.init_add()
        self.holdpct = pct
        self.dbhandle = punchclock.PCT_PunchClock.get_timeDB_handle(self.holdpct)
        llists = punchclock.PCT_PunchClock.get_tasklists(self.holdpct)
        self.d_list = llists[0]
        self.h_list = llists[1]
        ...
        self.processphoto=tk.PhotoImage(file='process.gif')
        self.atpb = tk.Button(self.holdatl, image=self.processphoto, command=self.process_add_task )
        ...
        # project, task, alert, show, autostart,guisort rptsort, taskyr is current, get back taskid
        twidth = 25
    ...
    def val_task(self,reason,value,widget,ix):
        ...
        self.holdparent.after_idle(lambda v=reason: self.edtfld[i][0].config(validate=v))
        if ok == True:
    ...
    # (tid, ttid, pnm, tnm, tat, tash, tast, tsrt, ttim, tdt) = self.tasklist[ix]
        ...
        for f in self.edtfld:
            v = f[0].get()
            elist.append(v)
        self.n_list = [None,None,None,None,None,None,None,None,None,None]
        ...
        if len(value) > 0:
            ...
            if ok: 
                if int(value) > int(PCT_TimeDB.PCT_TimeDB.get_config_item(self.holddbh,"DAYLEN"))/60:
                    ok = False
            self.valtoplvl.after_idle(lambda r=reason: self.valent.config(validate=r))
            if ok == True:
    ...
    def misc_tasks_display(self,pcthandle,dbhandle): # date field
        ...
        self.miscqb = tk.Button(self.holdmisctl, image=self.miscqphoto,
                command=lambda lltl = self.holdmisctl \
                :punchclock.PCT_PunchClock.popupquit(pcthandle,lltl))
        ...
        ToolTip.ToolTip(self.misccalb, anchor='e', text="Use a Calendar Widget to select the date")
        ...
        dl=tk.Label(self.holdmisctl, textvariable=self.dt, width=23, anchor='w')
        ...
        for day in daytype:
            ...
            command=lambda ltl = self.holdmisctl, lrbv = self.rbdcv :self.process_misc_dayt(ltl,lrbv),
            borderwidth = 2, width=self.tlen, state=tk.NORMAL, offrelief=tk.RAISED, relief=tk.FLAT))
            self.rbbd[len(self.rbbd) - 1].grid(row=r+1,column=2)
    def process_misc_dayt(self,tl,rbv):
        dayix = rbv.get()
        if dayix == 2:
            self.misctiml=tk.Label(self.holdmisctl, text="Minutes", anchor='e', width=10) 
            self.misctiml.grid(row=8,column=1)
            self.misctim = tk.StringVar()
            self.misctime = tk.Entry(self.holdmisctl, width = 6, textvariable=self.misctim, validate = 'key')
            ...
            ToolTip.ToolTip(self.misctiml, anchor='e', text="Enter the time in minutes, not more than a working day's time.")
            ToolTip.ToolTip(self.misctime, anchor='e', text="Enter the time in minutes, not more than a working day's time.")
        else:
    ...
    def process_misc_task(self):
        ...
        if self.misctim != None:
            ...
            timno = int(PCT_TimeDB.PCT_TimeDB.get_config_item(self.holddbh,"DAYLEN"))
            if dayix == 1:
                timno /= 2
        tid = self.tlist[taskix][1]
        PCT_TimeDB.PCT_TimeDB.insert_misc_task_time(self.holddbh,tid,timno,dt[0:10])
    ...
    def lb_order_tasks(self,pcthandle,dbhandle,opt):
        ...
        lbpb.config(command=lambda relatedlb = lb:self.process_apply_new_order(relatedlb))
        ToolTip.ToolTip(lb, anchor='w', text="Select an item with the mouse and use the arrows to change the order")
        ...
        buttonup = tk.Button(holdlbtl, image=lbuphoto, command=lambda lb = lb:self.lb_move_up(lb))
        ...
        buttondn = tk.Button(holdlbtl, image=lbdphoto, command=lambda lb = lb:self.lb_move_down(lb))
        buttondn.grid(row=1, column=2, sticky=tk.E+tk.N)
        ToolTip.ToolTip(buttonup, anchor='w', text="Move the selection down")
        dictlist = {}
        buttons = [lbqphoto,lbpphoto,lbuphoto,lbdphoto,lbqb,lbpb,buttonup,buttondn,yscroll]
        for task in tasks:
            lb.insert(tk.END, task[0])
            dictlist[task[0]] = task[1]
        self.lbdict[lb] = [None,opt,pcthandle,holdlbtl,dictlist,buttons,h,len(tasks)]
    ...
    def edit_tasks_list(self,pcthandle,dbhandle):
        self.etllists = [None,None,None,None,None,None,None,pcthandle,dbhandle]
        ...
        vsb = tk.Scrollbar(holdetltl,orient="vertical", command=self.etl_OnVsb)
        self.etllists[1] = vsb
        ...
        lb2 = tk.Listbox(holdetltl, yscrollcommand=vsb.set, exportselection=1, width=tnwidth, selectmode=tk.SINGLE)
        ToolTip.ToolTip(lb2, anchor='e', text="Select the task to edit by clicking on the name")
        lb4 = tk.Listbox(holdetltl, yscrollcommand=vsb.set, exportselection=0, takefocus=0, relief=tk.FLAT)
        lb5 = tk.Listbox(holdetltl, yscrollcommand=vsb.set, exportselection=0, takefocus=0, relief=tk.FLAT)
        lb6 = tk.Listbox(holdetltl, yscrollcommand=vsb.set, exportselection=0, takefocus=0, relief=tk.FLAT)
    ...
    def edit_task(self,pcthandle,dbhandle,tid):
        ...
        etqb = tk.Button(holdettl, image=quitphoto,anchor='w',command=self.edit_task_quit)
        ...
        self.edtfldprocessbut = tk.Button(holdettl, image=processphoto, anchor='e',command=self.edit_task_update)
        self.edtfldprocessbut.grid(row=0, column=1)
        self.edtfldprocessbut.config(default = tk.DISABLED, state =  tk.DISABLED  )
        self.etlists[4] = self.edtfldprocessbut
        ToolTip.ToolTip(self.edtfldprocessbut, anchor='e', text="Process the changes")
    ...
    def edit_config_list(self,pcthandle,dbhandle):
        self.confglists = [None,None,None,None,None,None,None,pcthandle,dbhandle,None,None]
        # 0 is list of listboxes
        # 1 is common vscroll
        # 2 is toplevel
        # 3 is button image
        ...
        confglqb = tk.Button(holdconfgltl, image=confglqphoto, anchor='w',
                command=lambda lltl = holdconfgltl \
                :punchclock.PCT_PunchClock.popupquit(pcthandle,lltl))
        ...
        ToolTip.ToolTip(confglab, anchor='e', text="Add a new configuration key value pair")

        configs = self.DB_Handle.get_config()
        vsb = tk.Scrollbar(holdconfgltl,orient="vertical", command=self.confgl_OnVsb)
        ...
        lb0 = tk.Listbox(holdconfgltl, yscrollcommand=vsb.set, exportselection=1, width=25, selectmode=tk.SINGLE)
        ToolTip.ToolTip(lb0, anchor='e', text="Select the item to edit by clicking on the name")
        lb1 = tk.Listbox(holdconfgltl, yscrollcommand=vsb.set, exportselection=0, width=15, takefocus=0, relief=tk.FLAT)
        lb2 = tk.Listbox(holdconfgltl, yscrollcommand=vsb.set, exportselection=0, width=50, takefocus=0, relief=tk.FLAT)
    ...
    def edit_config(self,pcthandle,dbhandle,key):
        ...
        etqb = tk.Button(holdconfgtl, image=quitphoto,anchor='w',command=self.edit_config_quit)
        etqb.grid(row=0, column=0)
        self.confglists[2] = etqb
        ToolTip.ToolTip(etqb, anchor='e', text="Leave the window")
        processphoto=tk.PhotoImage(file='process.gif')
        self.confglists[3] = processphoto
        self.edtfldprocessbut = tk.Button(holdconfgtl, image=processphoto, anchor='e',command=self.edit_config_update)
        self.edtfldprocessbut.grid(row=0, column=1)
        self.edtfldprocessbut.config(default = tk.DISABLED, state =  tk.DISABLED  )
    ...
    def add_config(self,pcthandle,dbhandle):
        ...
        processphoto=tk.PhotoImage(file='process.gif')
        self.confglists[3] = processphoto
        self.edtfldprocessbut = tk.Button(holdconfgtl, image=processphoto, anchor='e',command=self.add_config_insert)
        self.edtfldprocessbut.grid(row=0, column=1)
        self.edtfldprocessbut.config(default = tk.DISABLED, state =  tk.DISABLED  )
        self.confglists[4] = self.edtfldprocessbut
        ToolTip.ToolTip(self.edtfldprocessbut, anchor='e', text="Process the addition")
        self.edtfldvalopt = []
        self.holdparent = self.confglists[0][0]
        twidth = 25
